...
TOTAL_FIELDS: int = len(OUTPUT_SCHEMA) - 1  # 17
...
class LLMParamExtractor:
    ...
    async def extract(
        ...
        if llm_result is not None:
            ...
            # 填充缺失的品牌/市场信息
            if not llm_result.get("brand") and brand:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return f"""你是一个专业的空调参数提取器。你的任务是从用户提供的产品文本中提取所有空调(AC)相关参数。

## 规则
1. 输出必须是**严格 JSON 格式**，只输出 JSON，不要包含任何其他文字说明、Markdown 包裹或代码块标记。
2. 所有数值字段必须是正确的类型（int / float / str / null），不要用字符串表示数字。
3. 如果文本中不包含某个字段的信息，**必须设为 null**，不要假设或编造任何值。
4. **注意单位转换**：
   - 制冷量/制热量必须用 **W（瓦特）**
   - 如果原文给出的是匹数（如 1.5匹），请换算为 W（1匹 ≈ 2500W，1.5匹 ≈ 3500W，2匹 ≈ 5000W）
   - 如果原文给出的是 BTU（如 9000BTU/h），请换算为 W（1 BTU/h ≈ 0.293W）
   - 如果原文给出的是 kW（如 2.8kW），请换算为 W（2.8kW → 2800W）
   - 噪音使用 dB 数值
   - 风量使用 m³/h
5. **品牌和型号**：从文本中提取品牌（brand）和型号（model），如无法提取设为 null。
6. **confidence**：根据你对提取结果的信心给出 0.0 ~ 1.0 的分数。
...
请严格按照上述 schema 输出 JSON。"""
    ...
    def _fallback_to_regex(
        ...
        # 填入元信息
        if not extracted.get("brand") and brand:
