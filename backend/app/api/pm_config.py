...
def get_perf_defaults(
    market: str = Query(..., description="目标市场，如'越南'、'通用'"),
    capacity: str = Query(..., description="冷量段，如'07K'、'09K'、'12K'、'18K'、'24K'"),
    db: Session = Depends(get_db),
    current_user: User = Depends(_require_auth),
):
    """获取指定市场+冷量段的性能默认参数"""
    items = (
        db.query(PerfDefault)
        .filter(PerfDefault.market == market, PerfDefault.capacity_range == capacity)
        .order_by(PerfDefault.sort_order, PerfDefault.id)
        .all()
    )
    return {
        "items": [
