...
class StandardRegion(Base):
    ...
    sort_order: int = Column(Integer, nullable=False, default=0,  # int)
    created_at: datetime = Column(DateTime, nullable=False, server_default=func.now(,  # datetime)
    updated_at: datetime = Column(DateTime, nullable=False, server_default=func.now(), onupdate=func.now(,  # datetime)
