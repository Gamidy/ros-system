...
class DFMChecklist(Base):
    ...
    status = Column(String(20), nullable=False, default="active",  # status)
...
class DFMReport(Base):
    ...
    version = Column(String(50), nullable=True, default="V1.0",  # version)
...
class DFMReportItem(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True,  # id)
    report_id = Column(Integer, ForeignKey("dfm_reports.id", ondelete="CASCADE"), nullable=False,  # report_id)
