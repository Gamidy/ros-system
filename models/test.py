...
class TestRequest(Base):
    ...
    requester = Column(String(50), nullable=False,  # requester)
    ...
    target_date = Column(Date, nullable=True,  # target_date)
    completed_date = Column(Date, nullable=True,  # completed_date)
    ng_count = Column(Integer, default=0, comment="不合格项数")
    result_summary = Column(Text, nullable=True,  # result_summary)
    # ---- Phase 6 增强字段 ----
...
class TestResult(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True,  # id)
    test_request_id = Column(Integer, ForeignKey("test_requests.id"), nullable=False,  # test_request_id)
    ...
    tested_by = Column(String(50), nullable=True,  # tested_by)
    tested_at = Column(DateTime, nullable=True,  # tested_at)
    # ---- 多租户 ----
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
...
class MQVerification(Base):
    ...
    part_name = Column(String(200), nullable=True,  # part_name)
    project_code = Column(String(50), nullable=True,  # project_code)
    ...
    pass_items = Column(Integer, default=0,  # pass_items)
    fail_items = Column(Integer, default=0,  # fail_items)
    result_report = Column(Text, nullable=True,  # result_report)
    verified_by = Column(String(50), nullable=True,  # verified_by)
    verified_at = Column(Date, nullable=True,  # verified_at)
    # ---- 多租户 ----
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
    created_at = Column(DateTime, server_default=func.now(,  # created_at)
...
class Certification(Base):
    ...
    submit_date = Column(Date, nullable=True,  # submit_date)
    approved_date = Column(Date, nullable=True,  # approved_date)
...
class Prototype(Base):
    ...
    produced_date = Column(Date, nullable=True,  # produced_date)
    test_date = Column(Date, nullable=True,  # test_date)
    result = Column(String(20), nullable=True, comment="pass/fail")
    remark = Column(Text, nullable=True,  # remark)
    # ---- 多租户 ----
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
...
class QualityIssue(Base):
    ...
    assigned_to = Column(String(50), nullable=True,  # assigned_to)
    target_date = Column(Date, nullable=True,  # target_date)
    closed_date = Column(Date, nullable=True,  # closed_date)
    # ---- 多租户 ----
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
    created_at = Column(DateTime, server_default=func.now(,  # created_at)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(,  # updated_at)
...
class ECR(Base):
    ...
    submitted_by = Column(String(50), nullable=True,  # submitted_by)
    approved_by = Column(String(50), nullable=True,  # approved_by)
    # ---- 多租户 ----
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
    created_at = Column(DateTime, server_default=func.now(,  # created_at)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(,  # updated_at)
...
class ECN(Base):
    ...
    ecr_id = Column(Integer, ForeignKey("ecrs.id"), nullable=True,  # ecr_id)
    title = Column(String(200), nullable=False,  # title)
    product_code = Column(String(50), nullable=True,  # product_code)
