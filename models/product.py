...
class Product(Base):
    ...
    platform_id = Column(Integer, ForeignKey("platforms.id"), nullable=False,  # platform_id)
...
class Market(Base):
    ...
    # ── 结束新增 ──
    is_active = Column(String(5), default="true",  # is_active)
    # ── 制热能效指标 ──
...
class Version(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True,  # id)
    product_id = Column(Integer, ForeignKey("products.id"), nullable=False,  # product_id)
    version_no = Column(String(50), nullable=False, comment="版本号，如 V1.0, V1.1, V2.0")
    status = Column(SAEnum(VersionStatus), default=VersionStatus.DRAFT, nullable=False,  # status)
...
class ManufacturingVariant(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True,  # id)
    version_id = Column(Integer, ForeignKey("versions.id"), nullable=False,  # version_id)
