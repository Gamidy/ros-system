...
class ECRRequest(Base):
    ...
    ecr_type = Column(String(30), nullable=False, default=ECRType.OTHER.value,  # ecr_type
                      comment=f"变更类型: {[e.value for e in ECRType]}")
    reason = Column(Text, nullable=False, comment="变更原因")
    urgency = Column(String(20), nullable=False, default=ECRUrgency.MEDIUM.value,  # urgency
    ...
    status = Column(String(20), nullable=False, default=ECRStatus.DRAFT.value,  # status
...
class ECO(Base):
    ...
    status = Column(String(20), nullable=False, default=ECOStatus.DRAFT.value,  # status
...
class ECOItem(Base):
    ...
    change_type = Column(String(20), nullable=False,  # change_type
                         comment=f"变更类型: {[e.value for e in ECOChangeType]}")
    object_type = Column(String(20), nullable=False,  # object_type
