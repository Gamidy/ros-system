...
class CertificationRequirement(Base):
    ...
    status = Column(String(20), nullable=False, default=CertExecutionStatus.PENDING.value,  # status
                    comment=f"状态: {[e.value for e in CertExecutionStatus]}")
    source_type = Column(String(20), nullable=False, default=CertRequirementSource.TARGET_MARKET.value,  # source_type
...
class CertificationProject(Base):
    ...
    status = Column(String(20), nullable=False, default=CertProjectStatus.PLANNING.value,  # status
                    comment=f"状态: {[e.value for e in CertProjectStatus]}")
    planned_start_date = Column(Date, nullable=True,  # planned_start_date)
    planned_end_date = Column(Date, nullable=True,  # planned_end_date)
    actual_start_date = Column(Date, nullable=True,  # actual_start_date)
...
class CertificationSample(Base):
    ...
    status = Column(String(20), nullable=False, default=CertSampleStatus.PENDING.value,  # status
                    comment=f"状态: {[e.value for e in CertSampleStatus]}")
    submitted_date = Column(Date, nullable=True,  # submitted_date)
    remark = Column(Text, nullable=True,  # remark)
    org_id = Column(Integer, ForeignKey("organizations.id"), nullable=True, comment="所属组织ID")
    created_at = Column(DateTime, server_default=func.now(,  # created_at)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(,  # updated_at)
...
class CertificationResult(Base):
    ...
    status = Column(String(20), nullable=False, default=CertResultStatus.DRAFT.value,  # status
                    comment=f"状态: {[e.value for e in CertResultStatus]}")
    result_date = Column(Date, nullable=True,  # result_date)
    summary = Column(Text, nullable=True,  # summary)
...
class Certificate(Base):
    ...
    status = Column(String(20), nullable=False, default=CertificateStatus.ACTIVE.value,  # status
