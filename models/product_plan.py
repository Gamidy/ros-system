...
class ProductPlan(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4(,  # id)
...
class ProductPlanProjectLink(Base):
    ...
    product_plan_id = Column(String(36), ForeignKey("product_plans.id", ondelete="CASCADE"), nullable=False,  # product_plan_id)
    ...
    created_at = Column(DateTime, default=func.now(,  # created_at)
...
class ProductRequirement(Base):
    ...
    id = Column(String(36), primary_key=True, default=uuid4_str,  # id)
    ...
    created_at = Column(DateTime, default=func.now(,  # created_at)
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now(,  # updated_at)
