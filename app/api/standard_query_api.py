...
router = APIRouter(prefix="/standards", tags=["标准知识库"])
...
def _row_to_out(row) -> StandardOut:
    ...
    # row is a SQLAlchemy Row (from select() + add_columns()), not ORM instance
    # Row supports __getitem__: row[0]=Standard, row[1]=StandardRegion, row[2]=StandardCategory
    from sqlalchemy.engine import Row as _SARow
    std: Standard = row[0] if isinstance(row, (list, tuple, _SARow)) else row
    region = getattr(row, 'StandardRegion', None) if not isinstance(row, (list, tuple, _SARow)) else \
        row[1] if len(row) > 1 else None
    category = getattr(row, 'StandardCategory', None) if not isinstance(row, (list, tuple, _SARow)) else \
