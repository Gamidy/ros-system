...
def login(req: LoginRequest, request: Request, db: Session = Depends(get_db)) -> Token:
    ip = request.client.host if request.client else "unknown"
    now = time.time()
    _login_attempts[ip] = [t for t in _login_attempts.get(ip, []) if now - t < LOGIN_WINDOW]
    if len(_login_attempts[ip]) >= MAX_LOGIN_ATTEMPTS:
        raise HTTPException(status_code=429, detail="登录尝试过于频繁，请60秒后重试")
