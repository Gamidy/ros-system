...
# ═══════════════ 库存列表（根路由，兼容前端直接调用 /api/inventory） ═══════════════

@router.get("", response_model=dict)
def list_inventory_root(
    ...
    """库存列表（兼容前端）"""
    q = db.query(Inventory).filter(Inventory.is_deleted == 0)
    if warehouse_id:
        q = q.filter(Inventory.warehouse_id == warehouse_id)
    if search:
        like = f"%{search}%"
        q = q.filter(or_(Inventory.part_code.like(like), Inventory.part_name.like(like)))
    total = q.count()
    items = q.order_by(Inventory.id.desc()).offset((page - 1) * page_size).limit(page_size).all()
