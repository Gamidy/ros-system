<template>
  <div class="pm-workspace">
    ...
    >
      <!-- 抽屉顶部校验错误提示 -->
      <el-alert
        v-if="currentTabErrors.length > 0"
        :title="`「${currentTabName}」有 ${currentTabErrors.length} 项未完成`"
        type="error"
        show-icon
        :closable="false"
      ...
      >
        <template #default>
          <div v-for="d in currentTabErrors" :key="d.field" style="font-size:13px;line-height:1.8">
            · {{ d.message }}
          </div>
        </template>
      </el-alert>
      <el-tabs v-model="activeTab" type="border-card" class="drawer-tabs">
        ...
        <el-tab-pane name="overview_market">
          <template #label>
            <el-tooltip :disabled="tabStatus.overview_market.valid" placement="top" popper-class="tab-error-tooltip">
              <template #content>
                <div v-for="d in tabStatus.overview_market.detailedErrors" :key="d.field" class="tab-error-item">
                  ⚠ {{ d.message }}
                </div>
              </template>
              <span>
        ...
        <el-tab-pane name="technical">
          <template #label>
            <el-tooltip :disabled="tabStatus.technical.valid" placement="top" popper-class="tab-error-tooltip">
              <template #content>
                <div v-for="d in tabStatus.technical.detailedErrors" :key="d.field" class="tab-error-item">
                  ⚠ {{ d.message }}
                </div>
              </template>
              <span>
        ...
        <el-tab-pane name="cost">
          <template #label>
            <el-tooltip :disabled="tabStatus.cost.valid" placement="top" popper-class="tab-error-tooltip">
              <template #content>
                <div v-for="d in tabStatus.cost.detailedErrors" :key="d.field" class="tab-error-item">
                  ⚠ {{ d.message }}
                </div>
              </template>
              <span>
        ...
        <el-tab-pane name="team">
          <template #label>
            <el-tooltip :disabled="tabStatus.team.valid" placement="top" popper-class="tab-error-tooltip">
              <template #content>
                <div v-for="d in tabStatus.team.detailedErrors" :key="d.field" class="tab-error-item">
                  ⚠ {{ d.message }}
                </div>
              </template>
              <span>
...
// ═══════════════════════════════════════════════
// 计算属性 (必须在所有响应式数据定义之后？不，必须在之后。但按规范，computed可在任意位置)
// ═══════════════════════════════════════════════
...
// 当前Tab的错误详情（用于抽屉顶部错误alert）
const currentTabErrors = computed<TabValidationDetail[]>(() => {
  return tabStatus[activeTab.value]?.detailedErrors || []
})

// 当前Tab的中文名称
const currentTabName = computed(() => {
  const names: Record<string, string> = {
    overview_market: '项目概述与市场',
    technical: '技术要求',
    cost: '成本核算',
    team: '团队与职责',
  }
...
// Tab 校验状态
const tabStatus = reactive<Record<string, { valid: boolean; errors: string[]; detailedErrors: TabValidationDetail[] }>>({
  overview_market: { valid: false, errors: [], detailedErrors: [] },
  technical: { valid: false, errors: [], detailedErrors: [] },
  cost: { valid: false, errors: [], detailedErrors: [] },
  team: { valid: false, errors: [], detailedErrors: [] },
})
...
function validateOverviewMarket(): void {
  const errs: string[] = []
  const details: TabValidationDetail[] = []
  const f = projectForm
  if (!f.program_id) { errs.push('所属项目群'); details.push({ field: 'program_id', label: '所属项目群', message: '请选择所属项目群' }) }
  if (!f.leader_id) { errs.push('项目负责人'); details.push({ field: 'leader_id', label: '项目负责人', message: '请选择项目负责人' }) }
  if (!f.product_type) { errs.push('产品类型'); details.push({ field: 'product_type', label: '产品类型', message: '请选择产品类型' }) }
  if (!f.target_market) { errs.push('目标市场'); details.push({ field: 'target_market', label: '目标市场', message: '请选择目标市场' }) }
  if (!f.capacity_range) { errs.push('能力段'); details.push({ field: 'capacity_range', label: '能力段/冷量', message: '请填写能力段/冷量' }) }
  if (!f.refrigerant) { errs.push('制冷剂'); details.push({ field: 'refrigerant', label: '制冷剂', message: '请选择制冷剂' }) }
  if (!f.voltage_freq) { errs.push('电压频率'); details.push({ field: 'voltage_freq', label: '电压频率', message: '请选择电压频率' }) }
  if (!f.start_date) { errs.push('立项日期'); details.push({ field: 'start_date', label: '立项日期', message: '请选择立项日期' }) }
  if (!f.target_end_date) { errs.push('计划完成日期'); details.push({ field: 'target_end_date', label: '计划完成日期', message: '请选择计划完成日期' }) }
  const hasCustomerReq = customerReqTable.some(row =>
    row.category || row.description || row.source || row.tech_impact || row.market_impact
  )
  if (!hasCustomerReq) { errs.push('客户关键需求'); details.push({ field: 'customerReqTable', label: '客户关键需求', message: '客户关键需求至少填写一行' }) }
  tabStatus.overview_market.valid = errs.length === 0
  tabStatus.overview_market.errors = errs
  tabStatus.overview_market.detailedErrors = details
}
...
function validateTechnical(): void {
  ...
  if (!hasTargetValue) { errs.push('核心技术指标'); details.push({ field: 'corePerfTable', label: '核心技术指标', message: '核心性能参数至少一行填写目标值' }) }
  if (safetyComplianceTable.length === 0) { errs.push('安全合规标准'); details.push({ field: 'safetyComplianceTable', label: '安全合规标准', message: '安全合规标准未加载，请先选择目标市场' }) }
  const hasMaterial = materialComponentTable.some(row => row.name || row.spec)
  if (!hasMaterial) { errs.push('物料部件清单'); details.push({ field: 'materialComponentTable', label: '物料部件清单', message: '物料部件清单至少填写一行' }) }
  tabStatus.technical.valid = errs.length === 0
  tabStatus.technical.errors = errs
  tabStatus.technical.detailedErrors = details
}
...
function validateCost(): void {
  ...
  const details: TabValidationDetail[] = []
  const f = projectForm
  if (devCostGrandTotal.value <= 0) { errs.push('开发费用'); details.push({ field: 'devCostGrandTotal', label: '开发费用', message: '开发费用合计必须大于0' }) }
  if (f.fob_price === undefined || f.fob_price === null || Number(f.fob_price) <= 0) { errs.push('目标出厂价FOB'); details.push({ field: 'fob_price', label: '目标出厂价FOB', message: '请填写目标出厂价FOB' }) }
  if (f.bom_cost_target === undefined || f.bom_cost_target === null || Number(f.bom_cost_target) <= 0) { errs.push('目标BOM成本'); details.push({ field: 'bom_cost_target', label: '目标BOM成本', message: '请填写目标BOM成本' }) }
  tabStatus.cost.valid = errs.length === 0
  tabStatus.cost.errors = errs
  tabStatus.cost.detailedErrors = details
}
...
function validateTeam(): void {
  ...
  if (!hasUser) { errs.push('团队成员'); details.push({ field: 'teamTable', label: '团队成员', message: '团队至少选择一名成员' }) }
  if (teamTable.length === 0) { errs.push('团队表格'); details.push({ field: 'teamTable', label: '团队表格', message: '团队表格不能为空' }) }
  tabStatus.team.valid = errs.length === 0
  tabStatus.team.errors = errs
  tabStatus.team.detailedErrors = details
}
...
async function submitProposal() {
  ...
  if (!allValid) {
    ...
    if (firstInvalid) {
      ...
      ElMessage.warning(`请完善「${currentTabName.value}」信息：${errors.join('、')}`)
    }
    return
  }
...
.tab-error-tooltip .tab-error-item {
  font-size: 12px;
  line-height: 1.6;
  color: #f56c6c;
  white-space: nowrap;
