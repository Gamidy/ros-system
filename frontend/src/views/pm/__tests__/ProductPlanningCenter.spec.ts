...
describe('ProductPlanningCenter — Workspace Merge', () => {
  ...
  it('存在 activeTab 状态变量', () => {
    const wrapper = createWrapper()
    expect((wrapper.vm as unknown as Record<string, unknown>).activeTab).toBeDefined()
  })

  it('默认 activeTab 为 plans', () => {
    const wrapper = createWrapper()
    expect((wrapper.vm as unknown as Record<string, unknown>).activeTab).toBe('plans')
  })

  it('保留 filterStatus 原有状态变量', () => {
    const wrapper = createWrapper()
    expect((wrapper.vm as unknown as Record<string, unknown>).filterStatus).toBeDefined()
  })

  it('存在 fetchStatistics 和 fetchRoadmap 方法', () => {
    const wrapper = createWrapper()
    expect(typeof (wrapper.vm as unknown as Record<string, unknown>).fetchStatistics).toBe('function')
    expect(typeof (wrapper.vm as unknown as Record<string, unknown>).fetchRoadmap).toBe('function')
  })
