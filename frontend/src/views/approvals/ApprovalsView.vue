      ...
      <template v-if="detailItem">
        <div v-loading="detailLoading" style="min-height: 60px;">
          ...
          <div class="detail-grid">
            ...
            <span class="detail-label">申请类型:</span><span>{{ typeMap[detailItem.request_type] || detailItem.request_type }}</span>
          ...
          <!-- ECR 详情 -->
          <template v-if="detailItem.request_type === 'ecr' && ecrData">
            <h3 style="margin: 20px 0 12px 0; color: #409eff;">ECR 变更内容</h3>
            <div class="detail-grid">
              <span class="detail-label">ECR编号:</span><span>{{ ecrData.code }}</span>
              <span class="detail-label">变更类型:</span><span>{{ ecrData.ecr_type }}</span>
              <span class="detail-label">紧急度:</span>
              <span>
                <el-tag :type="urgencyTag(ecrData.urgency)" size="small">{{ urgencyMap[ecrData.urgency] || ecrData.urgency }}</el-tag>
              </span>
              <span class="detail-label">变更原因:</span><span style="white-space: pre-wrap;">{{ ecrData.reason }}</span>
              <span class="detail-label">变更描述:</span><span style="white-space: pre-wrap;">{{ ecrData.description || '-' }}</span>
              <span class="detail-label">提交人:</span><span>{{ ecrData.submitter_name || '-' }}</span>
              <span class="detail-label">影响产品:</span>
              <span>
                <template v-if="ecrData.affected_products">
                  <div v-for="(v, k) in ecrData.affected_products" :key="k">{{ k }}: {{ v }}</div>
                </template>
                <span v-else>-</span>
              </span>
              <span class="detail-label">影响文档:</span>
              <span>
                <template v-if="ecrData.affected_documents">
                  <div v-for="(v, k) in ecrData.affected_documents" :key="k">{{ k }}: {{ v }}</div>
                </template>
                <span v-else>-</span>
              </span>
              <span class="detail-label">附件:</span>
              <span>
                <template v-if="ecrData.attachments && ecrData.attachments.length > 0">
                  <div v-for="att in ecrData.attachments" :key="att.id" style="margin: 4px 0;">
                    <el-tag size="small" type="info">{{ att.file_type }}</el-tag>
                    <span style="margin: 0 8px;">{{ att.file_name }}</span>
...
const typeMap: Record<string, string> = { ecr: 'ECR变更', ecn: 'ECN变更', bom: 'BOM变更', certification: '认证', register: '账号申请' }
const urgencyMap: Record<string, string> = { urgent: '紧急', high: '高', medium: '中', low: '低' }
function urgencyTag(v: string) {
  const m: Record<string, string> = { urgent: 'danger', high: 'warning', medium: 'info', low: '' }
  return m[v] || 'info'
}
...
// 根据申请类型加载详情
async function _fetchRequestDetail(row: any) {
  if (!row) return
  if (row.request_type === 'ecr' && row.request_id) {
    detailLoading.value = true
    try {
      const res = await api.get(`/ecr/${row.request_id}`)
      ecrData.value = res.data
    } catch { /* ignore */ }
...
function downloadAttachment(att: any) {
  window.open(`/api/ecr/${att.ecr_id}/attachments/${att.id}/download`, '_blank')
}
function doApprove() {
